...
def add_takeover_visit_charts(df_d, df_m, st, prefix=""):
    chart_config = [
        ...
        {"title": "Top5 Site Visit", "daily": "IM-Top5Visit-Daily", "mtd": "IM-%Top5-MTD", "target": 95},
        {"title": "Ticket Closed", "daily": "IM-Closed-Daily", "mtd": "IM-%Closing-MTD", "target": 95},
        {"title": "RCA Validation", "daily": "IM-RCA-Daily", "mtd": "IM-%RCAValidation-MTD", "target": 30},
    ]
...
def app_tab1():
    ...
    df_daily, df_mtd, _, _ = load_ticketing_data()
...
def app_tab3():
    st.header("📌 Incident Cluster - MTTR P90 by Severity")

    # Load all data
    _, _, df_daily, df_mtd = load_ticketing_data()

    if df_daily.empty or df_mtd.empty:
        st.warning("Cluster-level ticketing data not found or empty.")
        return
    ...
    with c1:
        selected_month = st.selectbox("Month", months, index=months.index(latest_month), key="month_tab3")
    with c2:
        selected_year = st.selectbox("Year", years, key="year_tab3")
    with c3:
        selected_regional = st.selectbox("Regional", regionals, key="regional_tab3")

    # Filter NOPs by Regional
    filtered_nops = df_daily[df_daily["Regional"] == selected_regional]["NOP"].dropna().unique()
    filtered_nops = sorted(filtered_nops)
    with c4:
        selected_nop = st.selectbox("NOP", filtered_nops, key="nop_tab3")

    # Filter Clusters by NOP
    filtered_clusters = df_daily[
        (df_daily["Regional"] == selected_regional) &
        (df_daily["NOP"] == selected_nop)
    ]["Cluster TO"].dropna().unique()
    filtered_clusters = sorted(filtered_clusters)
    with c5:
        selected_cluster = st.selectbox("Cluster TO", filtered_clusters, key="cluster_tab3")
    ...
    df_d = df_daily[
        ...
        (df_daily["Regional"] == selected_regional) &
        (df_daily["NOP"] == selected_nop) &
        (df_daily["Cluster TO"] == selected_cluster)
    ].copy()
    ...
    df_m = df_mtd[
        ...
        (df_mtd["Regional"] == selected_regional) &
        (df_mtd["NOP"] == selected_nop) &
        (df_mtd["Cluster TO"] == selected_cluster)
    ].copy()
...
def app_tab4():
    ...
    # Load cluster data only
    _, _, df_daily, df_mtd = load_ticketing_data()

    if df_daily.empty or df_mtd.empty:
        st.warning("Cluster ticketing data not found or empty.")
        return

    from datetime import datetime

    # Extract sorted filters
    months = sorted(df_daily["Month"].dropna().unique().tolist(), key=lambda x: datetime.strptime(x, "%B"))
    years = sorted(df_daily["Year"].dropna().unique())
    regionals = sorted(df_daily["Regional"].dropna().unique().tolist())
    ...
    with c1:
        selected_month = st.selectbox("Month", months, index=months.index(default_month), key="month_tab4")
    with c2:
        selected_year = st.selectbox("Year", years, key="year_tab4")
    with c3:
        selected_regional = st.selectbox("Regional", regionals, key="regional_tab4")

    # Cascading NOP
    available_nops = df_daily[df_daily["Regional"] == selected_regional]["NOP"].dropna().unique().tolist()
    available_nops = sorted(available_nops)
    with c4:
        selected_nop = st.selectbox("NOP", available_nops, key="nop_tab4")

    # Cascading Cluster TO
    filtered_df = df_daily[
        (df_daily["Regional"] == selected_regional) &
        (df_daily["NOP"] == selected_nop)
    ]
    available_clusters = filtered_df["Cluster TO"].dropna().unique().tolist()
    available_clusters = sorted(available_clusters)
    with c5:
        selected_cluster = st.selectbox("Cluster TO", available_clusters, key="cluster_tab4")
...
def app():
    ...
    # Now define the tabs
    tab1, tab2, tab3, tab4 = st.tabs(["📍 Incident", "📍 Event", "📍 Incident Cluster", "📍 Event Cluster"])
    with tab1:
        app_tab1()
    with tab2:
        app_tab2()
